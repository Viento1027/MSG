...
def parse_args():
    parser = argparse.ArgumentParser("HSI Finetune")
    ...
    # ----- paths
    parser.add_argument('--save_path', default='ft_results', type=str)
...
def run_one_dataset(args, dataset_name):
    """Run fine-tuning on a single dataset (train/val/test pipeline)."""
    ...
    with open(main_log_path, "a") as f:
        f.write(f"\n\n=== Started Fine-tuning at {datetime.now()} ===\n")
    ...
    for run_seed in range(1, args.runs + 1):
        ...
        if args.is_train:
            ...
            criterion = torch.nn.CrossEntropyLoss(
                weight=compute_class_weights(train_loader, device),
                label_smoothing=0.1
            )
